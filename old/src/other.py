...
async def calculate_by_population(ctx):
    ...
    datas["objectif"] = int(
        await discord_input(ctx, "Entrez l'objectif de nombre d'habitants : ")
    )
...
def convert_country_name_channel(old_name: str):
    ...
    for i in range(len(old_name)):
        if i == 0:
            ...
            (old_name[i - 1] not in string.ascii_letters)
            and (old_name[i - 1] in car_sal2)
            and (old_name[i] in string.ascii_letters)
            and (old_name[i - 1] not in accents)
            and (old_name[i] not in car_sal2)
        ):
...
def take_points(player_id, amount, type: int = 1):
    if type == 1:
        cur.execute(
            "SELECT pol_points FROM inventory WHERE player_id = ?", (player_id,)
        )
...
def calculate_construction_cost(datas, total_area, building):
    ...
    mur_cost = (
        mur_price[0] * total_area
    )  # Calculer le coût des murs (hypothèse simplifiée)
    ...
    fondations_cost = prix_fonda * (
        building["surface_net"] * building["profondeur_fondation"]
    )  # Hypothèse simplifiée du coût des fondations au mètre carré
...
async def eco_logger(
    code, amount, user1: discord.Member, user2: discord.Member = None, type: int = 1
):
    ...
    if code.startswith("M"):
        if code == "M1":
            embed = discord.Embed(
                title="Nouvelle transaction entre joueurs",
                description=f":moneybag: L'utilisateur {user1.name} ({user1.id}) a donné {amount} à {user2.name} ({user2.id}).",
                color=money_color_int,
            )
        elif code == "M2":
            embed = discord.Embed(
                title="<a:NE_Alert:1261090848024690709> Ajout d'argent",
                description=f":moneybag: L'utilisateur {user1.name} ({user1.id}) s'est fait ajouter {amount} par {user2.name} ({user2.id}).",
                color=money_color_int,
            )
        elif code == "M3":
            embed = discord.Embed(
                title="<a:NE_Alert:1261090848024690709> Argent défini",
                description=f":moneybag: L'utilisateur {user1.name} ({user1.id}) s'est fait définir son argent à {amount} par {user2.name} ({user2.id}).",
                color=money_color_int,
            )
        elif code == "M4":
            embed = discord.Embed(
                title="Argent payé",
                description=f":moneybag: L'utilisateur {user1.name} ({user1.id}) a payé {amount} à la banque.",
                color=money_color_int,
            )
        elif code == "M5":
            embed = discord.Embed(
                title="<a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> Retrait d'argent",
                description=f":moneybag: L'utilisateur {user1.name} ({user1.id}) s'est fait retirer {amount} par {user2.name} ({user2.id}).",
                color=money_color_int,
            )
        elif code == "MR":
            embed = discord.Embed(
                title="<a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> Reset de l'économie",
                description=f":moneybag: L'utilisateur {user1.name} ({user1.id}) a réinitialisé l'économie.",
                color=money_color_int,
            )
        elif code == "MRR":
            embed = discord.Embed(
                title="<a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> Tentative de reset de l'économie par un membre",
                description=f":moneybag: L'utilisateur {user1.name} ({user1.id}) a essayé de réinitialiser l'économie.",
                color=money_color_int,
            )
        ...
        if code == "P1":
            embed = discord.Embed(
                title=f"<a:NE_Alert:1261090848024690709> {p_type} ajoutés",
                description=f":blue_circle: L'utilisateur {user1.name} ({user1.id}) s'est fait ajouter {amount} {p_type} par {user2.name} ({user2.id}).",
                color=points_color_int,
            )
        elif code == "P2":
            embed = discord.Embed(
                title=f"<a:NE_Alert:1261090848024690709> {p_type} définis",
                description=f":blue_circle: L'utilisateur {user1.name} ({user1.id}) s'est fait définir ses {p_type} à {amount} par {user2.name} ({user2.id}).",
                color=points_color_int,
            )
        elif code == "P3":
            embed = discord.Embed(
                title=f"{p_type} utilisé",
                description=f":blue_circle: L'utilisateur {user1.name} ({user1.id}) a utilisé {amount} {p_type}.",
                color=points_color_int,
            )
        elif code == "P4":
            embed = discord.Embed(
                title=f"<a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> {p_type} retirés",
                description=f":blue_circle: L'utilisateur {user1.name} ({user1.id}) s'est fait retirer {amount} {p_type} par {user2.name} ({user2.id}).",
                color=points_color_int,
            )
        elif code == "PR":
            embed = discord.Embed(
                title=f"<a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> Reset des {p_type}",
                description=f":blue_circle: L'utilisateur {user1.name} ({user1.id}) a réinitialisé les {p_type}.",
                color=points_color_int,
            )
        elif code == "PRR":
            embed = discord.Embed(
                title=f"<a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> <a:NE_Alert:1261090848024690709> Tentative de reset des {p_type} par un membre non autorisé",
                description=f":blue_circle: L'utilisateur {user1.name} ({user1.id}) a essayé de réinitialiser les {p_type}.",
                color=points_color_int,
            )
    else:
        print("Erreur")
...
async def set_money_func(ctx, user: discord.Member, amount: int):
    ...
    if result is None:
        ...
        cur.execute(
            "UPDATE inventory SET balance = ? WHERE country_id = ?", (amount, user.id)
        )
...
async def set_points_func(ctx, user: discord.Member, amount: int, type: int = 1):
    if type == 1:
        ...
        if result is None:
            cur.execute(
                "INSERT INTO inventory (country_id, pol_points) VALUES (?, ?)",
                (user.id, amount),
            )
        else:
            cur.execute(
                "UPDATE inventory SET pol_points = ? WHERE country_id = ?",
                (amount, user.id),
            )
    else:
        cur.execute(
            "SELECT diplo_points FROM inventory WHERE country_id = ?", (user.id,)
        )
        result = cur.fetchone()
        if result is None:
            cur.execute(
                "INSERT INTO inventory (country_id, diplo_points) VALUES (?, ?)",
                (user.id, amount),
            )
        else:
            cur.execute(
                "UPDATE inventory SET diplo_points = ? WHERE country_id = ?",
                (amount, user.id),
            )
    conn.commit()
