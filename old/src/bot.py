...
async def get_user_role(ctx):
    await ctx.message.reply(
        f"get_user_role {ctx.author.id} {ctx.author.name}: {get_player_role(ctx)}"
    )
...
async def give(ctx, user: discord.Member, amount: Union[int, str]):
    ...
    if recipient_balance is None:
        cur.execute(
            "INSERT INTO inventory (player_id, balance) VALUES (?, ?)", (user.id, 0)
        )

    # Effectuer la transaction en mettant à jour les données dans la base de données
    cur.execute(
        "UPDATE inventory SET balance = balance + ? WHERE player_id = ?",
        (payment_amount, str(user.id)),
    )
    cur.execute(
        "UPDATE inventory SET balance = balance - ? WHERE player_id = ?",
        (payment_amount, str(author.id)),
    )
    conn.commit()
...
async def remove_pp(ctx, user: discord.Member, amount: Union[int, str]):
    ...
    take_points(user.id, payment_amount, 1)
    embed = discord.Embed(
        title="Opération réussie",
...
async def remove_pd(ctx, user: discord.Member, amount: Union[int, str]):
    ...
    take_points(user.id, payment_amount, 2)
    embed = discord.Embed(
        title="Opération réussie",
...
async def set_money(ctx, user: discord.Member, amount: int):
    ...
    if result is None:
        ...
        cur.execute(
            "UPDATE inventory SET balance = ? WHERE player_id = ?", (amount, user.id)
        )
...
async def set_pp(ctx, user: discord.Member, amount: int):
    ...
    if result is None:
        ...
        cur.execute(
            "UPDATE inventory SET pol_points = ? WHERE player_id = ?", (amount, user.id)
        )
...
async def lead_eco(ctx):
    cur.execute(
        "SELECT player_id, balance FROM inventory ORDER BY balance DESC LIMIT 10"
    )
    leaderboard = cur.fetchall()

    embed = discord.Embed(
        title="Classement des pays les plus riches :", color=money_color_int
    )
    for i, (user_id, balance) in enumerate(leaderboard, 1):
        ...
        embed.add_field(
            name=f"{i}. {username}",
            value=f":moneybag: **{convert(str(balance))}** argent",
            inline=False,
        )
...
async def lead_pp(ctx):
    cur.execute(
        "SELECT player_id, pol_points FROM inventory ORDER BY pol_points DESC LIMIT 10"
    )
    leaderboard = cur.fetchall()

    embed = discord.Embed(
        title="Classement des pays avec le plus de pp :", color=p_points_color_int
    )
    for i, (user_id, points) in enumerate(leaderboard, 1):
        ...
        embed.add_field(
            name=f"{i}. {username}",
            value=f":blue_circle: **{points}** points",
            inline=False,
        )
...
async def lead_pd(ctx):
    cur.execute(
        "SELECT player_id, diplo_points FROM inventory ORDER BY diplo_points DESC LIMIT 10"
    )
    leaderboard = cur.fetchall()

    embed = discord.Embed(
        title="Classement des pays avec le plus de pp :", color=d_points_color_int
    )
    for i, (user_id, points) in enumerate(leaderboard, 1):
        user = ctx.guild.get_member(int(user_id))
        if user:
        ...
        embed.add_field(
            name=f"{i}. {username}",
            value=f":purple_circle: **{points}** points diplomatiques",
            inline=False,
        )
...
async def usines(ctx, type, user: discord.Member = None):
    ...
    if type.lower() == "all":
        ...
        embed = discord.Embed(
            ...
            description=f"L'utilisateur a **{str(get_usine(user.id, type, 0))}** usines de type {type}.",
        )
    await ctx.send(embed=embed)
...
async def batiments(ctx, bat_type: int, type, user: discord.Member = None):
    ...
    if type.lower() == "all":
        ...
        for i in range(1, max_lvl + 1):  # On parcourt les niveaux du type de bâtiment
            embed.description += (
                f"Niveau {i} : {str(get_usine(user.id, i, bat_type))}\n"
            )
    else:
        embed = discord.Embed(
...
async def groq_chat(ctx, *, message):
    ...
    if last_groq_query_time:
        if (
            datetime.now(timezone.utc) - last_groq_query_time < timedelta(seconds=180)
            and ctx.author.id not in bi_admins_id
        ):
